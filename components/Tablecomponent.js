...
export default function Tablecomponent(props) {
  ...
  const handlechangefilter = (e) => {
    if (
      ...
      updatedblogs = updatedblogs.filter((item) => {
        return (
          item.URL.toLowerCase().indexOf(e.target.value.toLowerCase()) !== -1
        );
      });
  ...
  const addindex = (urlval) => {
    ...
    axios.post(INDEX_URL + "/addindex", { url, type }).then((res) => {
      ...
      setstatus(
        JSON.stringify(JSON.stringify(res.data.urlNotificationMetadata))
      );
    });
  };
              ...
              {urllist
                ...
                .map((row, key) => (
                  <StyledTableRow key={key}>
                    ...
                    <StyledTableCell align="right">
                      <Button onClick={() => viewstatus(row.URL)}>
                        Get status
                      </Button>
                    </StyledTableCell>
                    <StyledTableCell align="right">
                      <Button onClick={() => addindex(row.URL)}>
                        Add to indexind
                      </Button>
                    </StyledTableCell>
                    <StyledTableCell align="right">
                      <Button onClick={() => removeindex(row.URL)}>
                        Delete from indexind
                      </Button>
                    </StyledTableCell>
                  </StyledTableRow>
                ))}
