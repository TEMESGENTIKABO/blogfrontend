...
const MediumEditor = dynamic(() => import("../../Editor/MediumEditor"), {
  ssr: false,
});
...
const CreateBlog = ({ router }) => {
  ...
  const handleEmailEditorData = () => {
    emailEditorRef.current.editor.exportHtml((data) => {
      ...
      let str = JSON.stringify(body_element.innerHTML).replace(
        /(<([^>]+)>)/gi,
        ""
      );
    ...
    return (
      ...
      tags.map((t, i) => (
        <ListItem key={i}>
          <ListItemButton
            role={undefined}
            onClick={handleTagsToggle(t._id)}
            dense
          >
            <ListItemIcon>
              <Checkbox
        ...
        <Paper style={{ borderRadius: "10px" }}>
          <div style={{ padding: "20px" }}>
            ...
            <div className="form-group">
              ...
              {EditorjsState && (
                <MediumEditor
                  editorjson={handlejsondata}
                  value={
                    localStorage.getItem("jsonblog") !== "undefined"
                      ? JSON.parse(localStorage.getItem("jsonblog"))
                      : {}
                  }
                />
              )}
  ...
  return (
    <div className="container-fluid pb-5">
      <div className="form-group">
        <Box>
          <Toggle
            EmailEditorState={EmailEditorState}
            handlePublish={publishBlog}
            handlequill={handlequilleditor}
            editorjs={handleeditorjs}
            emailEditor={handleEmailEditor}
          />
        </Box>
      </div>
      {EmailEditorState && (
